export default function ProgrammingInfoTree() {
  return (
    <section
      id="programming-info-tree"
      className="mt-16 font-[YEKAN] rounded-[40px] bg-gradient-to-tr from-[#8a2387] via-[#e94057] to-[#f27121] pb-4"
    >
      <h3 className="relative top-5 mb-20 text-center  text-4xl font-medium text-white after:absolute after:left-1/2 after:bottom-[-5px] after:h-1 after:w-1/3 after:-translate-x-1/2 after:rounded-full after:bg-white">
        درخت برنامه نویسی
      </h3>

      <div className="relative mx-auto my-12 max-w-[1080px] px-5">
        <div className="absolute left-1/2 top-5 h-[84%] w-1 -translate-x-1/2 bg-white" />

        <div className="flex justify-start">
          <section
            className="relative w-[calc(50%-40px)] rounded-md bg-white p-5 rtl"
            dir="rtl"
          >
            <i className="absolute right-[-60px] top-4 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-[17px] text-red-400 shadow-[0_0_0_4px_#fff,0_3px_0_4px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(0,0,0,0.08)] icon" />
            <div className="flex items-center justify-between">
              <span className="text-[22px] font-semibold">
                تیتر 1: ساختار الگوریتمی
              </span>
            </div>
            <p className="my-2.5">
              رنامه نویسی ابزار پیشرفت است که مهارت تحلیل دادهها و طراحی
              الگوریتمهای نوین را به توسعه دهندگان ارائه می کند در ساختار منطقی
              کدها موفقیت پروژه های نرم افزاری تضمین می گردد
            </p>
            <div className="flex items-center justify-between">
              <a
                href="#"
                className="rounded bg-orange-600 px-4 py-2 font-normal text-white transition hover:scale-95"
              >
                بیشتر بدانید
              </a>
            </div>
          </section>
        </div>

        <div className="flex justify-end">
          <section
            className="relative w-[calc(50%-40px)] rounded-md bg-white p-5 rtl "
            dir="rtl"
          >
            <i className="absolute left-[-60px] top-4 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-[17px] text-red-400 shadow-[0_0_0_4px_#fff,0_3px_0_4px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(0,0,0,0.08)] icon" />
            <div className="flex items-center justify-between">
              <span className="text-[22px] font-semibold">
                تیتر 2: طراحی واسط کاربری
              </span>
            </div>
            <p className="my-2.5">
              ی واسط کاربری نقش اساسی ایفا می کند که به کارگیری مفهوم گرافیک
              مدرن ارتباط کاربر را با سیستم تسهیل می کند در تجربه دیجیتال نوآوری
              و سادگی عملکرد تضمین می شود
            </p>
            <div className="flex items-center justify-between">
              <a
                href="#"
                className="rounded bg-orange-600 px-4 py-2 font-normal text-white transition hover:scale-95"
              >
                بیشتر بدانید
              </a>
            </div>
          </section>
        </div>

        <div className="flex justify-start">
          <section
            className="relative w-[calc(50%-40px)] rounded-md bg-white p-5 rtl"
            dir="rtl"
          >
            <i className="absolute right-[-60px] top-4 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-[17px] text-red-400 shadow-[0_0_0_4px_#fff,0_3px_0_4px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(0,0,0,0.08)] icon" />
            <div className="flex items-center justify-between">
              <span className="text-[22px] font-semibold">
                تیتر 3: بهینه سازی عملکرد
              </span>
            </div>
            <p className="my-2.5">
              بهینه سازی عملکرد کدها موجبات افزایش کارایی سیستم را فراهم می کند
              با اجرای الگوریتمهای روزآمد کاهش زمان پاسخگویی و افزایش بهره مندی
              کاربران در پروژه های نرم افزاری تضمین می گردد
            </p>
            <div className="flex items-center justify-between">
              <a
                href="#"
                className="rounded bg-orange-600 px-4 py-2 font-normal text-white transition hover:scale-95"
              >
                بیشتر بدانید
              </a>
            </div>
          </section>
        </div>

        <div className="flex justify-end">
          <section
            className="relative w-[calc(50%-40px)] rounded-md bg-white p-5 rtl"
            dir="rtl"
          >
            <i className="absolute left-[-60px] top-4 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-[17px] text-red-400 shadow-[0_0_0_4px_#fff,0_3px_0_4px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(0,0,0,0.08)] icon" />
            <div className="flex items-center justify-between">
              <span className="text-[22px] font-semibold">
                تیتر 4: امنیت نرم افزاری
              </span>
            </div>
            <p className="my-2.5">
              امنیت نرم افزاری اعتماد کاربران در سیستم های محسوب می شود با پیاده
              سازی استانداردهای دقیق حفاظت اطلاعات و جلوگیری دسترسی غیرمجاز در
              برنامه های توسعه یافته اعتبار پروژه تضمین می گردد
            </p>
            <div className="flex items-center justify-between">
              <a
                href="#"
                className="rounded bg-orange-600 px-4 py-2 font-normal text-white transition hover:scale-95"
              >
                بیشتر بدانید
              </a>
            </div>
          </section>
        </div>

        <div className="flex justify-start">
          <section
            className="relative w-[calc(50%-40px)] rounded-md bg-white p-5 rtl"
            dir="rtl"
          >
            <i className="absolute right-[-60px] top-5 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-[17px] text-red-400 shadow-[0_0_0_4px_#fff,0_3px_0_4px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(0,0,0,0.08)] icon" />
            <div className="flex items-center justify-between">
              <span className="text-[22px] font-semibold">
                تیتر 3: بهینه سازی عملکرد
              </span>
            </div>
            <p className="my-2.5">
              بهینه سازی عملکرد کدها موجبات افزایش کارایی سیستم را فراهم می کند
              با اجرای الگوریتمهای روزآمد کاهش زمان پاسخگویی و افزایش بهره مندی
              کاربران در پروژه های نرم افزاری تضمین می گردد
            </p>
            <div className="flex items-center justify-between">
              <a
                href="#"
                className="rounded bg-orange-600 px-4 py-2 font-normal text-white transition hover:scale-95"
              >
                بیشتر بدانید
              </a>
            </div>
          </section>
        </div>
      </div>
    </section>
  );
}
